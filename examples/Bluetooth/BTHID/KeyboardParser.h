...
uint8_t KbdRptParser::HandleLockingKeys(USBHID *hid, uint8_t key) {
  ...
  if (old_keys != kbdLockingKeys.bLeds && hid) {
    BTHIDs *pBTHIDs = reinterpret_cast<BTHIDs *> (hid); // A cast the other way around is done in BTHIDs.cpp
    pBTHIDs->setLeds(kbdLockingKeys.bLeds); // Update the LEDs on the keyboard
  }
