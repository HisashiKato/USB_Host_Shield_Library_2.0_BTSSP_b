...
void setup() {
  ...
  bthids.SetReportParser(KEYBOARD_PARSER_ID, &keyboardPrs);
  bthids.SetReportParser(MOUSE_PARSER_ID, &mousePrs);

  // If "Boot Protocol Mode" does not work, then try "Report Protocol Mode"
  // If that does not work either, then uncomment PRINTREPORT in BTHIDs.cpp to see the raw report
  bthids.setProtocolMode(USB_HID_BOOT_PROTOCOL); // Boot Protocol Mode
  //bthids.setProtocolMode(HID_RPT_PROTOCOL); // Report Protocol Mode
  ...
  // Set SSP Data.
  bthids.setConnectAddress(ssp_bdaddr);
  bthids.setPairedLinkkey(ssp_link_key);
...
#ifdef ENABLE_PAIR
  if ((bthids.linkkeyNotification() == true)&&(stored_ssp_status == false)) {
    // Get SSP Data.
    bthids.getConnectedAddressHex(ssp_bdaddr);
    bthids.getPairedLinkkeyHex(ssp_link_key);
