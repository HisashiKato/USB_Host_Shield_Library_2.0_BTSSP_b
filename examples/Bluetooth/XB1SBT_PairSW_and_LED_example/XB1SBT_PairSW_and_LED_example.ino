...
void setup() {
//---------------
    pinMode(extBtnPin, INPUT_PULLUP); //active Low
...
//---------------
    pairingButtonCheck();
    pairModeSwitching();
    setLedstatus(Xb1s.btStatus());
    blinkLed(ledStatus);
//---------------

  if (Xb1s.btStatus() == BT_PAIRING_IN_PROGRESS) {
...
void setLedstatus(int btstatus) {
    switch(btstatus) {
        case BT_INIT:
            ledStatus = LED_ON;
            break;
        case BT_CONNECT_WAIT:
            ...
            break;
        case BT_CONNECT_IN_PROGRESS:
            ledStatus = BLINK_MEDIUM;
            break;
        case BT_CONNECTED_TO_DEVICE:
            ledStatus = LED_OFF;
            break;
        case BT_PAIRING_WAIT:
            ledStatus = BLINK_FAST;
            break;
        case BT_PAIRING_IN_PROGRESS:
            ledStatus = BLINK_MEDIUM;
            break;
        default:
            ledStatus = LED_OFF;
