...
public:
        ...
        bool connected() {
                ...
                noSignalTime = millis() - BTHIDs::getLastMessageTime();

                // xbox one s controller sends the battery status every 20 seconds.
                if ((noSignalTime > 21000) && (XB1SBTParser::disconnectRequest != true)){
#ifdef DEBUG_USB_HOST
                        Notify(PSTR("\r\nno response from controller"), 0x80);
#endif
                        XB1SBTParser::disconnectRequest = true; //not yet use ToDo
#ifdef DEBUG_USB_HOST
//                        Notify(PSTR("\r\ndisconnect()"), 0x80);
#endif
//                        BTHIDs::disconnect(); // still not work ToDo
                }

//              if ((BTHIDs::connected) && (!XB1SBTParser::disconnectRequest)){
                if (BTHIDs::connected){
                        return true;
                } else {
                        return false;
